...
def bispectrumi(y, nlag=None, nsamp=None, overlap=None,
    ...
    Args:
        ...
        nsamp: samples per segment [default: row dimension of y]
    ...
    s = 0
    for k in np.arange(nrecord):
        x = y[ind].ravel(order='F')
        x = x - np.mean(x)
        ind = ind + int(nadvance)

        for j in np.arange(nlag + 1):
            z = x[range(nsamp - j)] * x[range(j, nsamp)]
            for i in np.arange(j, nlag + 1):
    ...
    if wind != -1:
        ...
        for k in np.arange(-nlag, nlag + 1):
