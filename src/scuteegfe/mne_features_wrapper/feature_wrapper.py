    ...
    def feature_smooth(self,data,smooth_type="mv_av_filter",window_size=10):
        ...
        Args:
            ...
            smooth_type:                 str         "mv_av_filter"   move average filter
                                                     "lsd"             linear dynamic system (LDS) approach
            window_size:                 int
        ...
        for i_feature in range(n_feature):
            for i_channel in range(n_channel):
                   ...
                   if smooth_type=="mv_av_filter":
                       smoothed_data[:,i_channel,i_feature] = self.moving_average_filter(data[:,i_channel,i_feature], window_size)
                   if  smooth_type=="lsd":
                       smoothed_data[:,i_channel,i_feature] = self.lsd_KalmanFilter(data[:,i_channel,i_feature], window_size)
